...
step       ← test step records (one per step)
teardown   ← teardown command records
summary    ← final result (pass/fail counts, duration)
```
...
jq -r 'select(.type=="step") | "[\(.index)] \(.judge_method) \(.judge_verdict): \(.judge_reason)"' trace.jsonl
...
2. **Failing transition steps** — if a transition step (no judge_prompt) returns non-zero, it FAILs immediately. Make sure setup commands succeed.

3. **FAIL stops everything** — when a step FAILs, subsequent steps are not executed. Teardown still runs.

4. **Timeout** — default 30s per step. Long-running commands (builds, installs) need explicit `timeout: 120` or higher.
